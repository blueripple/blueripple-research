...
module BlueRipple.Data.KeyedF where
...
-- NB: This representation is..annoying.  It's not even a functor, though
-- the map Set -> Set which takes a set to the Free Abelian Group generated by
-- the elements of the set and then forgets the Abelian structure, is a
-- Monad and we will want that join.  But the natural representation, a list
-- of (Int, a) pairs is tricky as well.  How do we regroup?  That requires
-- at least (Eq a) and that will make it impossible to define even Functor.
-- SO we'll use this and implement "join" by hand
data KeyWeights a where
  KeyWeights :: Eq a => (a -> Int) -> KeyWeights a
...
kwUnit :: a -> KeyWeights a
kwUnit a = KeyWeights $ \x -> if a == x then 1 else 0

kwJoin :: KeyWeights (KeyWeights a) ->

kwOne :: KeyWeights a
kwOne = KeyWeights (const 1)

kwZero :: KeyWeights a
kwZero = KeyWeights (const 0)
