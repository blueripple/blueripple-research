...
parse1D name m = do
  ...
  ni <- fmap (\x -> x - 1) $ maybe (Left $ "No parameters with name " <> name <> " in parse1D?") Right $ FL.fold (FL.premap fst FL.maximum) indexed
...
parse2D name m = do
  ...
  let getI (i, _) = i
      getJ (_, j) = j
  ni <- fmap (\x -> x - 1) $ maybe (Left $ "No parameters with name " <> name <> " in parse2D?") Right $ FL.fold (FL.premap (getI . fst) FL.maximum) indexed
  nj <- fmap (\x -> x - 1) $ maybe (Left $ "No parameters with name " <> name <> " in parse2D?") Right $ FL.fold (FL.premap (getJ . fst) FL.maximum) indexed
...
parse3D name m = do
  ...
  let getI (i, _, _) = i
      getJ (_, j, _) = j
      getK (_, _, k) = k
  ni <- fmap (\x -> x - 1) $ maybe (Left $ "No parameters with name " <> name <> " in parse3D?") Right $ FL.fold (FL.premap (getI . fst) FL.maximum) indexed
  nj <- fmap (\x -> x - 1) $ maybe (Left $ "No parameters with name " <> name <> " in parse3D?") Right $ FL.fold (FL.premap (getJ . fst) FL.maximum) indexed
  nk <- fmap (\x -> x - 1) $ maybe (Left $ "No parameters with name " <> name <> " in parse3D?") Right $ FL.fold (FL.premap (getK . fst) FL.maximum) indexed
...
parse4D name m = do
  ...
  let getI (i, _, _, _) = i
      ...
      getK (_, _, k, _) = k
      getL (_, _, _, l) = l
  ni <- fmap (\x -> x - 1) $ maybe (Left $ "No parameters with name " <> name <> " in parse4D?") Right $ FL.fold (FL.premap (getI . fst) FL.maximum) indexed
  nj <- fmap (\x -> x - 1) $ maybe (Left $ "No parameters with name " <> name <> " in parse4D?") Right $ FL.fold (FL.premap (getJ . fst) FL.maximum) indexed
  nk <- fmap (\x -> x - 1) $ maybe (Left $ "No parameters with name " <> name <> " in parse4D?") Right $ FL.fold (FL.premap (getK . fst) FL.maximum) indexed
  nl <- fmap (\x -> x - 1) $ maybe (Left $ "No parameters with name " <> name <> " in parse4D?") Right $ FL.fold (FL.premap (getL . fst) FL.maximum) indexed
