...
filterCcesAndPumsByYear f (BRE.CCESAndPUMS cces cps pums dd) = BRE.CCESAndPUMS (q cces) (q cps) (q pums) (q dd)
 where
  q ∷ (F.ElemOf rs BR.Year, FI.RecVec rs) ⇒ F.FrameRec rs → F.FrameRec rs
  q = F.filterFrame (f . F.rgetField @BR.Year)

aggregatePredictorsCDFld fldData =
  FMR.concatFold $
    FMR.mapReduceFold
      FMR.noUnpack
...
aggregatePredictorsCountyFld fldData =
  FMR.concatFold $
    FMR.mapReduceFold
      FMR.noUnpack
...
prepCensusDistrictData clearCaches cacheKey cdData_C = do
  ...
  BR.retrieveOrMakeFrame cacheKey deps $ \(cdData, stateAbbrs) → do
    let addRace5' =
        ...
        (cdDataSER, cdMissing) =
          FJ.leftJoinWithMissing @'[BR.StateFips] cdDataSER' $
...
modelDetails ∷ ∀ r. (K.KnitMany r, BR.CacheEffects r) ⇒ BR.CommandLine → K.Sem r () -- BR.StanParallel -> Bool -> K.Sem r ()
...
stateAnalysis ∷ ∀ r. (K.KnitMany r, BR.CacheEffects r) ⇒ BR.CommandLine → K.Sem r ()
  ...
  let ccesAndCPS2020_C = fmap (BRE.ccesAndCPSForYears [2020]) ccesAndCPSEM_C
      ...
      modelDM name =
        BRE.electionModelDM
          ...
          (psInfoDM name)
          ccesAndCPS2020_C
          (fmap (F.rcast @StatePostStratR) <$> acs2020_C)
...
deepDive2022CD ∷ ∀ r. (K.KnitMany r, BR.CacheEffects r) ⇒ BR.CommandLine → Text → Text → K.Sem r ()
...
deepDive2022SLD ∷ ∀ r. (K.KnitMany r, BR.CacheEffects r) ⇒ BR.CommandLine → Text → ET.DistrictType → Text → K.Sem r ()
...
deepDive2020CD ∷ ∀ r. (K.KnitMany r, BR.CacheEffects r) ⇒ BR.CommandLine → Text → Int → K.Sem r ()
...
deepDiveState ∷ ∀ r. (K.KnitMany r, BR.CacheEffects r) ⇒ BR.CommandLine → Text → K.Sem r ()
...
deepDive ∷ ∀ r. (K.KnitMany r, BR.CacheEffects r) ⇒ BR.CommandLine → Text → K.ActionWithCacheTime r (F.FrameRec PostStratR) → K.Sem r ()
...
psDemographicsInnerFld =
  let cnt = F.rgetField @BRC.Count
   ...
   in --      cntWgtdF f = (/) <$> cntWgtdSumF f <*> fmap realToFrac cntF
      FF.sequenceRecFold $
...
modelDiagnostics ∷ ∀ r. (K.KnitMany r, BR.CacheEffects r) ⇒ BR.CommandLine → K.Sem r () -- BR.StanParallel -> Bool -> K.Sem r ()
...
byCategoryColonnade
  ∷ ( F.ElemOf rs BRE.ModeledTurnout
...
cdDiagTableColonnade cas =
  let state' = F.rgetField @DT.StateAbbreviation
      ...
      rawDShare r = ratio (demVoters r) (demVoters r + repVoters r)
   in --      ccesDShare r = ratio (F.rgetField @BRE.PresDVotes r) (F.rgetField @BRE.PresDVotes r + F.rgetField @BRE.PresRVotes r)
      C.headed "State" (BR.toCell cas "State" "State" (BR.textToStyledHtml . state'))
...
newStateLegMapPosts ∷ ∀ r. (K.KnitMany r, BR.CacheEffects r) ⇒ BR.CommandLine → K.Sem r ()
newStateLegMapPosts cmdLine = do
  ...
  let postInfoPA = BR.PostInfo (BR.postStage cmdLine) (BR.PubTimes BR.Unpublished Nothing)
      ...
      spDistsPA =
        fmap (\x → (ET.StateLower, x)) ["88", "142", "144", "160"]
  ...
  let postInfoMI = BR.PostInfo (BR.postStage cmdLine) (BR.PubTimes BR.Unpublished Nothing)
      ...
      spDistsMI =
        ((ET.StateLower,) <$> ["38", "46", "54", "58", "81", "84"])
...
pwldByStateFld
  ∷ (F.ElemOf rs BR.StateAbbreviation, F.ElemOf rs DT.PopPerSqMile)
  ⇒ (F.Record rs → Int)
...
rescaleDensity
  ∷ (F.ElemOf rs DT.PopPerSqMile, Functor f)
  ⇒ Double
  → f (F.Record rs)
  → f (F.Record rs)
...
capa ∷ ∀ r. (K.KnitMany r, BR.CacheEffects r) ⇒ BR.CommandLine → K.Sem r ()
capa cmdLine = do
  ...
  let senateStates = Set.fromList ["CO", "AZ", "GA", "NV", "NH", "PA", "FL", "NC", "WI"]
      capaFilter r =
        not (state' r `Set.member` senateStates)
          && histDS r >= 0.47
          && histDS r <= 0.53
  ...
  let overlappingSLDs' threshold cdn overlaps' = do
        let
          -- numLD = Vector.length $ DO.populations overlaps'
      ...
      overlappingSLDs threshold sa cdn = do
        ...
        return $
          fmap (\(n, x) → (ET.StateUpper, n, x)) namedOverU
  ...
  let sld2PSMap =
        FL.fold (FL.premap (\r → (F.rcast @[BR.StateAbbreviation, ET.DistrictTypeC, ET.DistrictName] r, F.rgetField @TwoPartyDShare r)) FL.map) $
...
dobbs ∷ ∀ r. (K.KnitMany r, BR.CacheEffects r) ⇒ BR.CommandLine → K.Sem r ()
dobbs cmdLine = do
...
newCDModeled
  ...
  → K.Sem
      r
      ( K.ActionWithCacheTime r (F.FrameRec ModeledWithDRAndAggDemo)
  ...
  let mapGroup ∷ SB.GroupTypeTag (F.Record CDLocWStAbbrR) = SB.GroupTypeTag "CD"
      ...
      modelDM name =
        BRE.electionModelDM
          ...
          (psInfoDM name)
          ccesAndCPS2020_C
          (fmap (F.rcast @PostStratR) <$> rescaledProposed_C)
...
allCDsPost ∷ ∀ r. (K.KnitMany r, BR.CacheEffects r) ⇒ BR.CommandLine → K.Sem r ()
allCDsPost cmdLine = K.wrapPrefix "allCDsPost" $ do
  ...
  let oldCDOverlapsE ∷ (F.ElemOf rs BR.StateAbbreviation, F.ElemOf rs ET.DistrictName) ⇒ F.Record rs → Either Text Text
      oldCDOverlapsE r = do
        stateOverlaps ←
          maybe (Left $ "Failed to find stateAbbreviation=" <> state' r <> " in " <> show overlaps') Right $
            M.lookup (state' r) overlaps'
        cdOverlaps ←
          maybe (Left $ "Failed to find dist=" <> distr r <> " in " <> state' r <> " in " <> show overlaps') Right $
  ...
  districtsForClustering_C ←
    BR.retrieveOrMakeD "newMaps/allCDs/districtsForClustering.bin" (pure ()) $
      const $
        pure $
          BRDC.districtsForClustering @[DT.SexC, DT.CollegeGradC, DT.Race5C, DT.PopPerSqMile] @BRC.Count $
            fmap F.rcast demoModelDRA
  ...
  let districtCompare x y =
        let st = F.rgetField @BR.StateAbbreviation
  ...
  BR.brNewPost allCDsPaths postInfo "AllCDs" $ do
    ...
    let distKey r =
          F.rgetField @BR.StateAbbreviation r
            <> "-"
...
data SBCS = SBCS
  { sbcNational ∷ Map (Text, Text) (Either Text [DCC.SBCCategoryData DCC.SBComparison])
...
newCongressionalMapPosts ∷ ∀ r. (K.KnitMany r, BR.CacheEffects r) ⇒ BR.CommandLine → K.Sem r ()
newCongressionalMapPosts cmdLine = do
  ...
  let postInfoNC =
        BR.PostInfo
          ...
          ( BR.PubTimes
              (BR.Published $ Time.fromGregorian 2021 12 15)
              (Just $ BR.Published $ Time.fromGregorian 2022 05 13)
          )
  ...
  let postInfoTX =
        BR.PostInfo
          (BR.postStage cmdLine)
          ( BR.PubTimes
              (BR.Published $ Time.fromGregorian 2022 2 25)
              (Just $ BR.Published $ Time.fromGregorian 2022 05 13)
          )
...
plausibleSurpriseFilter
  ...
  → F.Record rs
  → Bool
plausibleSurpriseFilter cc brR draR r =
  categoryFilter cc brR draR [SafeD] [Tossup, LeanR] r
    || categoryFilter cc brR draR [SafeR] [Tossup, LeanD] r

implausibleSurpriseFilter
  ∷ (F.ElemOf rs TwoPartyDShare, F.ElemOf rs BRE.ModeledShare)
  ⇒ (F.Record rs → Bool)
  → (Int, Int)
  → (Int, Int)
  → F.Record rs
  → Bool
implausibleSurpriseFilter cc brR draR r =
  categoryFilter cc brR draR [SafeR, LeanR, Tossup] [SafeD] r
    || categoryFilter cc brR draR [SafeD, LeanD, Tossup] [SafeR] r

implausibleSafeDFilter
  ∷ (F.ElemOf rs TwoPartyDShare, F.ElemOf rs BRE.ModeledShare)
  ⇒ (F.Record rs → Bool)
  → (Int, Int)
  → (Int, Int)
...
demographicallyFavorableFilter
  ∷ (F.ElemOf rs TwoPartyDShare, F.ElemOf rs BRE.ModeledShare)
  ⇒ (F.Record rs → Bool)
  → (Int, Int)
  → (Int, Int)
...
demographicallyUnfavorableFilter
  ∷ (F.ElemOf rs TwoPartyDShare, F.ElemOf rs BRE.ModeledShare)
  ⇒ (F.Record rs → Bool)
  → (Int, Int)
  → (Int, Int)
...
data CategorizedDistricts f rs = CategorizedDistricts
...
categorizeDistricts
  ...
  → (Int, Int)
  → f (F.Record rs)
  → K.Sem r (CategorizedDistricts [] rs)
categorizeDistricts cc brR draR allDists = do
  let length' = FL.fold FL.length
      ...
      allFld =
        (\c r bc ps dd is ss → (c, r, CategorizedDistricts bc ps dd is ss))
          <$> FL.length
          <*> restFld
          <*> bothCloseFld
          <*> plausibleFld
  ...
  let findOverlaps
        ...
        ⇒ g (F.Record rs)
        → g (F.Record rs)
        → Set.Set (F.Record [ET.DistrictTypeC, ET.DistrictName])
      findOverlaps a b =
...
categorizeDistricts' cc brR draR cats allDists = do
  ...
  let findOverlaps
        ...
        ⇒ g (F.Record rs)
        → g (F.Record rs)
        → Set.Set (F.Record [BR.StateAbbreviation, ET.DistrictTypeC, ET.DistrictName])
      findOverlaps a b =
...
newStateLegMapAnalysis
  ∷ ∀ r
   . (K.KnitMany r, K.KnitOne r, BR.CacheEffects r)
  ⇒ BR.CommandLine
  → NewSLDMapsPostSpec
  → Bool
...
newStateLegMapAnalysis cmdLine postSpec interestingOnly ccesAndCPSEM_C acs_C cdDemo_C sldDemo_C = K.wrapPrefix "newStateLegMapAnalysis" $ do
  ...
  let modMid = round . (100 *) . MT.ciMid . F.rgetField @BRE.ModeledShare
      ...
      modelCompetitive n = brCompetitive || draCompetitive
       where
        draCompetitive = fromMaybe False $ fmap (between draShareRangeCD) $ M.lookup n cdDRAMap
  ...
  let overlapsMMap (dt, dn) = M.lookup dt (overlaps postSpec) >>= (\d → DO.overlapsOverThresholdForRowByName 0.5 d dn)
      tableCAS
        ∷ (F.ElemOf rs BRE.ModeledShare, F.ElemOf rs TwoPartyDShare, F.ElemOf rs ET.DistrictName, F.ElemOf rs ET.DistrictTypeC)
  ...
  K.logLE K.Info $
    "For "
      <> districtDescription postSpec
      <> " in "
      <> stateAbbr postSpec
      <> " there are "
      <> show (length interestingDistricts)
      <> " interesting districts."
  let districtOrder ra rb =
        let cd = compare (dType ra) (dType rb)
         in if cd /= EQ
              then cd
...
newCongressionalMapAnalysis _ cmdLine postSpec postInfo _ ccesAndCPSEM_C acs_C extantDemo_C proposedDemo_C = K.wrapPrefix "newCongressionalMapsAnalysis" $ do
  ...
  let
    -- ccesAndCPS2018_C = fmap (BRE.ccesAndCPSForYears [2018]) ccesAndCPSEM_C
  ...
  let addDistrict' r = r F.<+> ((ET.Congressional F.&: show (F.rgetField @ET.CongressionalDistrict r) F.&: V.RNil) ∷ F.Record [ET.DistrictTypeC, ET.DistrictName])
      addElexDShare r =
        let dv = F.rgetField @BRE.DVotes r
...
xyFold' labelFunc =
 ...
 where
  ...
  foldData =
    (\a wnh grads m d → (100 * realToFrac wnh / realToFrac a, 100 * realToFrac grads / realToFrac a, 100 * (m - 0.5), d))
      <$> allF
      <*> wnhF
      <*> gradsF
      <*> modelF
...
modelVsHistoricalTableCellStyle
  ∷ ( F.ElemOf rs BRE.ModeledShare
    , F.ElemOf rs TwoPartyDShare
    )
  ⇒ (Int, Int)
...
modelAndElectionScatter single title vc rows =
  let toVLDataRec =
        FVD.asVLData GV.Str "State"
          ...
          V.:& FVD.asVLData'
            [ ("Demographic Model", GV.Number . (* 100) . MT.ciMid)
      ...
      encModelLo =
        GV.position
          GV.Y
          [ GV.PName "Demographic Model (5% CI)"
          , GV.PmType GV.Quantitative
          , GV.PAxis [GV.AxTitle "Demographic Model"]
          , yScale
          ]
      encModelHi =
        GV.position
          GV.Y2
          [ GV.PName "Demographic Model (95% CI)"
          , GV.PmType GV.Quantitative
          , yScale
          ]
      encElection =
      ...
      encTooltips =
        GV.tooltips
          [ [GV.TName "District", GV.TmType GV.Nominal]
...
aggregatePredictorsInDistricts =
  FMR.concatFold $
    FMR.mapReduceFold
      FMR.noUnpack
